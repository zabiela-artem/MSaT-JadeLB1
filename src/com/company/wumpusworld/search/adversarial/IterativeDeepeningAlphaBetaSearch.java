package com.company.wumpusworld.search.adversarial;
...
public class IterativeDeepeningAlphaBetaSearch<S, A, P> implements AdversarialSearch<S, A> {
    ...
    public A makeDecision(S state) {
        ...
        do {
            ...
            for (A action : results) {
                ...
                if (logEnabled) logText.append(action).append("->").append(value).append(" ");
            }
            if (logEnabled) System.out.println(logText);
            if (newResults.size() > 0) {
                results = newResults.actions;
                if (!timer.timeOutOccurred()) {
                    if (hasSafeWinner(newResults.utilValues.get(0))) break; // exit from iterative deepening loop
    ...
    private static class ActionStore<A> {
        ...
        void add(A action, double utilValue) {
            int idx = 0;
            while (idx < actions.size() && utilValue <= utilValues.get(idx)) idx++;
