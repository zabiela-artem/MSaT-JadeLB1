...
public class RecursiveBestFirstSearch<S, A> implements SearchForActions<S, A>, Informed<S, A> {
    ...
    public RecursiveBestFirstSearch(ToDoubleFunction<Node<S, A>> evalFn, boolean avoidLoops, NodeExpander<S, A> nodeExpander) {
        this.evalFn = evalFn;
    ...
    private SearchResult<S, A> rbfs(Problem<S, A> p, Node<S, A> node, double node_f, double fLimit, int recursiveDepth) {
        ...
        // if problem.GOAL-TEST(node.STATE) then return SOLUTION(node)
        if (p.testSolution(node)) return getResult(null, node, fLimit);
        ...
        // if successors is empty then return failure, infinity
        if (successors.isEmpty()) return getResult(node, null, INFINITY);
        ...
        while (true) {
            ...
            // result, best.f <- RBFS(problem, best, min(f_limit, alternative))
            SearchResult<S, A> sr = rbfs(p, successors.get(bestIndex), f[bestIndex], Math.min(fLimit, f[altIndex]), recursiveDepth + 1);
    ...
    private SearchResult<S, A> getResult(Node<S, A> currNode, Node<S, A> solutionNode, double fCostLimit) {
        if (avoidLoops && currNode != null) explored.remove(currNode.getState());
