...
import com.company.logic.propositional.inference.OptimizedDPLL;
...
public class WumpusKB extends KnowledgeBase {
    ...
    public WumpusKB(int caveXDim, int caveYDim) {
        this(caveXDim, caveYDim, new OptimizedDPLL());
    }

    public WumpusKB(int caveXDim, int caveYDim, DPLL dpll) {
     ...
     */
    public WumpusKB(int caveXDim, int caveYDim, AgentPosition start, DPLL dpll) {
        this.start = start;
        this.dpll = dpll;
    ...
    public AgentPosition askCurrentPosition(int t) {
        ...
        else throw new IllegalStateException("Inconsistent KB, unable to determine current room orientation.");
        return current;
    }
    ...
    public Set<Room> askSafeRooms(int t) {
        ...
        for (int x = 1; x <= getCaveXDimension(); x++) {
            for (int y = 1; y <= getCaveYDimension(); y++) {
                ...
                if (ask(new ComplexSentence(newSymbol(LOCATION_VISITED, x, y), Connective.OR, newSymbol(OK_TO_MOVE_INTO, t, x, y)))) {
    ...
    public Set<Room> askSafeRooms(int t, Set<Room> visited) {
        ...
        for (int x = 1; x <= getCaveXDimension(); x++) {
            for (int y = 1; y <= getCaveYDimension(); y++) {
                Room r = new Room(x, y);
                if (visited.contains(r) || ask(newSymbol(OK_TO_MOVE_INTO, t, x, y))) safe.add(new Room(x, y));
            }
        }
        return safe;
    }
    ...
    public Set<Room> askNotUnsafeRooms(int t, Set<Room> visited) {
        ...
        for (int x = 1; x <= getCaveXDimension(); x++) {
            for (int y = 1; y <= getCaveYDimension(); y++) {
                Room r = new Room(x, y);
                if (visited.contains(r) || !ask(new ComplexSentence(Connective.NOT, newSymbol(OK_TO_MOVE_INTO, t, x, y))))
                    notUnsafe.add(r);
            }
        }
        return notUnsafe;
    ...
    public Set<Room> askPossibleWumpusRooms(int t) {
        ...
        for (int x = 1; x <= getCaveXDimension(); x++)
            for (int y = 1; y <= getCaveYDimension(); y++)
                if (!ask(new ComplexSentence(Connective.NOT, newSymbol(WUMPUS, x, y)))) possible.add(new Room(x, y));
        return possible;
    }
...
//				}
    ...
    public void makePerceptSentence(WumpusPercept p, int time) {
        if (p.isStench()) tell(newSymbol(PERCEPT_STENCH, time));
        else tell(new ComplexSentence(Connective.NOT, newSymbol(PERCEPT_STENCH, time)));

        if (p.isBreeze()) tell(newSymbol(PERCEPT_BREEZE, time));
        else tell(new ComplexSentence(Connective.NOT, newSymbol(PERCEPT_BREEZE, time)));

        if (p.isGlitter()) tell(newSymbol(PERCEPT_GLITTER, time));
        else tell(new ComplexSentence(Connective.NOT, newSymbol(PERCEPT_GLITTER, time)));

        if (p.isBump()) tell(newSymbol(PERCEPT_BUMP, time));
        else tell(new ComplexSentence(Connective.NOT, newSymbol(PERCEPT_BUMP, time)));

        if (p.isScream()) tell(newSymbol(PERCEPT_SCREAM, time));
        else tell(new ComplexSentence(Connective.NOT, newSymbol(PERCEPT_SCREAM, time)));
    }
    ...
    public void makeActionSentence(WumpusAction a, int time) {
        for (WumpusAction action : WumpusAction.values()) {
            if (action.equals(a)) tell(newSymbol(action.getSymbol(), time));
            else tell(new ComplexSentence(Connective.NOT, newSymbol(action.getSymbol(), time)));
        }
    }
    ...
    protected void tellAtemporalPhysicsSentences() {
        ...
        for (int i = 0; i < numRooms; i++) {
            for (int j = i + 1; j < numRooms; j++) {
                tell(new ComplexSentence(Connective.OR, new ComplexSentence(Connective.NOT, newSymbol(WUMPUS, (i / caveXDimension) + 1, (i % caveYDimension) + 1)), new ComplexSentence(Connective.NOT, newSymbol(WUMPUS, (j / caveXDimension) + 1, (j % caveYDimension) + 1))));
            }
        }
    }
    ...
    public void tellTemporalPhysicsSentences(int t) {
        ...
        for (int x = 1; x <= caveXDimension; x++) {
            for (int y = 1; y <= caveYDimension; y++) {
                tell(new ComplexSentence(newSymbol(LOCATION, t, x, y), Connective.IMPLICATION, new ComplexSentence(newSymbol(PERCEPT_BREEZE, t), Connective.BICONDITIONAL, newSymbol(BREEZE, x, y))));

                tell(new ComplexSentence(newSymbol(LOCATION, t, x, y), Connective.IMPLICATION, new ComplexSentence(newSymbol(PERCEPT_STENCH, t), Connective.BICONDITIONAL, newSymbol(STENCH, x, y))));
            }
        }
        ...
        for (int x = 1; x <= caveXDimension; x++) {
            for (int y = 1; y <= caveYDimension; y++) {
                ...
                tell(new ComplexSentence(newSymbol(LOCATION, t + 1, x, y), Connective.IMPLICATION, newSymbol(LOCATION_VISITED, x, y)));
            }
        }
    ...
    public void tellTemporalPhysicsSentences(int t, AgentPosition agentPosition) {
        ...
        // through the location fluent as follows. For any time step t
        // and any square [x,y], we assert
        tell(new ComplexSentence(newSymbol(LOCATION, t, agentPosition.getX(), agentPosition.getY()), Connective.IMPLICATION, new ComplexSentence(newSymbol(PERCEPT_BREEZE, t), Connective.BICONDITIONAL, newSymbol(BREEZE, agentPosition.getX(), agentPosition.getY()))));

        tell(new ComplexSentence(newSymbol(LOCATION, t, agentPosition.getX(), agentPosition.getY()), Connective.IMPLICATION, new ComplexSentence(newSymbol(PERCEPT_STENCH, t), Connective.BICONDITIONAL, newSymbol(STENCH, agentPosition.getX(), agentPosition.getY()))));
        ...
        for (int x = 1; x <= caveXDimension; x++) {
            for (int y = 1; y <= caveYDimension; y++) {
                ...
                tell(new ComplexSentence(newSymbol(OK_TO_MOVE_INTO, t, x, y), Connective.BICONDITIONAL,
                        ...
                        new ComplexSentence(new ComplexSentence(Connective.NOT, newSymbol(PIT, x, y)), Connective.AND, new ComplexSentence(Connective.NOT, new ComplexSentence(newSymbol(WUMPUS, x, y), Connective.AND, newSymbol(WUMPUS_ALIVE, t))))));
            }
        }

        // Rule about the arrow
        tell(new ComplexSentence(newSymbol(HAVE_ARROW, t + 1), Connective.BICONDITIONAL, new ComplexSentence(newSymbol(HAVE_ARROW, t), Connective.AND, new ComplexSentence(Connective.NOT, newSymbol(ACTION_SHOOT, t)))));

        // Rule about wumpus (dead or alive)
        tell(new ComplexSentence(newSymbol(WUMPUS_ALIVE, t + 1), Connective.BICONDITIONAL, new ComplexSentence(newSymbol(WUMPUS_ALIVE, t), Connective.AND, new ComplexSentence(Connective.NOT, newSymbol(PERCEPT_SCREAM, t + 1)))));
    ...
    private void tellSuccessorStateLocationAxiom(int t, int x, int y) {
        ...
        locDisjuncts.add(new ComplexSentence(newSymbol(LOCATION, t, x, y), Connective.AND, new ComplexSentence(new ComplexSentence(Connective.NOT, newSymbol(ACTION_FORWARD, t)), Connective.OR, newSymbol(PERCEPT_BUMP, t + 1))));
        if (x > 1) { // West room is possible
            locDisjuncts.add(new ComplexSentence(newSymbol(LOCATION, t, x - 1, y), Connective.AND, new ComplexSentence(newSymbol(FACING_EAST, t), Connective.AND, newSymbol(ACTION_FORWARD, t))));
        }
        if (y < caveYDimension) { // North room is possible
            locDisjuncts.add(new ComplexSentence(newSymbol(LOCATION, t, x, y + 1), Connective.AND, new ComplexSentence(newSymbol(FACING_SOUTH, t), Connective.AND, newSymbol(ACTION_FORWARD, t))));
        }
        if (x < caveXDimension) { // East room is possible
            locDisjuncts.add(new ComplexSentence(newSymbol(LOCATION, t, x + 1, y), Connective.AND, new ComplexSentence(newSymbol(FACING_WEST, t), Connective.AND, newSymbol(ACTION_FORWARD, t))));
        }
        if (y > 1) { // South room is possible
            locDisjuncts.add(new ComplexSentence(newSymbol(LOCATION, t, x, y - 1), Connective.AND, new ComplexSentence(newSymbol(FACING_NORTH, t), Connective.AND, newSymbol(ACTION_FORWARD, t))));
        }

        tell(new ComplexSentence(newSymbol(LOCATION, t + 1, x, y), Connective.BICONDITIONAL, Sentence.newDisjunction(locDisjuncts)));
    }
